...
namespace Ryujinx.Graphics.Gpu.Memory
{
    ...
    class BufferCache : IDisposable
    {
        ...
        private void CreateMultiRangeBuffer(MultiRange range)
        {
            ...
            for (int i = 0; i < range.Count; i++)
            {
                ...
                if (subRange.Address != MemoryManager.PteUnmapped)
                {
                    ...
                    BufferRange bufferRange = buffer.GetRange(alignedAddress, alignedSize);
        ...
        public void ForceDirty(MemoryManager memoryManager, ulong gpuVa, ulong size)
        {
            ...
            {
                MultiRange range = TranslateAndCreateBuffers(memoryManager, gpuVa, size);
        ...
        public bool CheckModified(MemoryManager memoryManager, ulong gpuVa, ulong size, out ulong outAddr)
        {
            ...
            if (!_modifiedCache.TryGetValue(alignedGpuVa, out BufferCacheEntry result) ||
                ...
                result.UnmappedSequence != result.Buffer.UnmappedSequence)
            {
                MultiRange range = TranslateAndCreateBuffers(memoryManager, alignedGpuVa, size);
                ulong address = range.GetSubRange(0).Address;
        ...
        private void CreateBufferAligned(ulong address, ulong size, ulong alignment)
        {
            ...
            if (overlapsCount != 0)
            {
                    ...
                    !overlap0.SparseCompatible)
                {
                    ...
                    CreateBufferAligned(address, newSize, sparseCompatible: true, overlaps, overlapsCount);
                }
            }
            else
            {
                // No overlap, just create a new buffer.
                Buffer buffer = new(_context, _physicalMemory, address, size, sparseCompatible: true);
        ...
        {
            MultiRange srcRange = TranslateAndCreateBuffers(memoryManager, srcVa, size);
            MultiRange dstRange = TranslateAndCreateBuffers(memoryManager, dstVa, size);
        ...
        public BufferRange GetBufferRangeTillEnd(MultiRange range, bool write = false)
        {
            ...
            {
                MemoryRange subRange = range.GetSubRange(0);
                return GetBuffer(subRange.Address, subRange.Size, write).GetRange(subRange.Address);
            }
        }
        ...
        public BufferRange GetBufferRange(MultiRange range, bool write = false)
        {
            ...
            {
                MemoryRange subRange = range.GetSubRange(0);
                return GetBuffer(subRange.Address, subRange.Size, write).GetRange(subRange.Address, subRange.Size);
            }
        }
